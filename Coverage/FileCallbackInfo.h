...
{
	FileCallbackInfo(const std::string& filename, PVOID processBasePtr) :
		filename(filename),
		registerLines(true),
		processBasePtr(processBasePtr)
	{
	...
	void SetBreakpoints(PVOID baseAddress, HANDLE process)
	{
		for (auto addr : breakpointsToSet)
		{
			...
			breakPoints.find(addr)->second.originalData = instruction;
